report 66683 "PHD Admission Letter"
{

    ApplicationArea = All;
    RDLCLayout = './Reports/SSR/phdAdmissionLetter.rdl';
    PreviewMode = PrintLayout;
    UsageCategory = ReportsAndAnalysis;

    dataset
    {
        dataitem(phdLetter; "ACA-Applic. Form Header")
        {
            ...
            begin
                progFee.Reset();
                progFee.SetRange(progFee.ProgCode, phdLetter."First Degree Choice");
                if progFee.Find('-') then begin
                    fee := progFee.intakeFee;
                end;
                Intake.Reset();
                Intake.SetRange(Intake."Code", phdLetter."Intake Code");
