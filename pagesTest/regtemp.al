page 84523 "ACA-Application RegTemp"
{
    CardPageID = "REG temp App";
    //InsertAllowed = false;
    ModifyAllowed = false;
    PageType = List;
    SourceTable = "ACA-Applic. Form Header";
    SourceTableView = WHERE(Status = FILTER(Open), "Process Application" = filter(false));
    ...
    layout
    {
        area(content)
        {
            repeater("General Information")
            {
                ...
                //     end;
                // }
                field("SRO UserID"; Rec."SRO UserID")
                {
                    ApplicationArea = All;
                }
