page 40027 "Applicants Domestic"
{
    Caption = 'Domestic Applicants ';
    CardPageID = "ACA-Application Form Header";
    InsertAllowed = false;
    ModifyAllowed = false;
    PageType = List;
    SourceTable = "ACA-Applic. Form Header";
    SourceTableView = WHERE(Status = FILTER(Open), "Process Application" = filter(false), "international student " = const(FALSE));// "Finance Cleared" = filter(true)
